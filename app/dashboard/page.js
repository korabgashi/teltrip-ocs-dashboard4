  ...
  const loadList = async () => {
    ...
    try {
      ...
      const enriched = await Promise.all(subscribers.map(async (s) => {
        ...
        const iccid = getICCID(s);

        const singleRes = await postJSON("/api/ocs/get-single-subscriber", {
          getSingleSubscriber: { subscriberId }
        });
        const sub = singleRes?.getSingleSubscriber || {};

        const usage = n(sub?.usedDataByte);
        const pkgSize = n(sub?.pckDataByte);
        const lastUsage = sub?.lastConnectionTime;

        const pkgRes = await postJSON("/api/ocs/list-subscriber-prepaid-packages", {
          listSubscriberPrepaidPackages: { subscriberId }
        });
        const packages = a(pkgRes?.listSubscriberPrepaidPackages?.packages);
        const mainPkg = packages[0] || {};

        const activated = mainPkg?.startDate;
        const expires = mainPkg?.endDate;
        const packageName = mainPkg?.prepaidPackageTemplateName;
        const subscriberCost = n(mainPkg?.subscriberCost);
        const resellerCost = n(mainPkg?.resellerCost);

        const profit = subscriberCost - resellerCost;
        const margin = subscriberCost ? (profit / subscriberCost) * 100 : 0;
    ...
    <div style={{ maxWidth: 1280, margin: "0 auto", padding: 24 }}>
      ...
      {rows.map((r, i) => (
        <div key={i} style={{
          ...
          <div>{cleanDate(r.activated)}</div>
          <div>{cleanDate(r.expires)}</div>
          <div>{cleanDate(r.lastUsage)}</div>
          <div>{gb(r.usage)}</div>
          <div>{gb(r.pkgSize)}</div>
